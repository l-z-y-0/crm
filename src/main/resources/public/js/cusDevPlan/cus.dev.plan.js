layui.use(['table','layer'],function(){
    ...
    var  tableIns = table.render({
        elem: '#saleChanceList',
        url : ctx+'/sale_chance/list?flg=1&state=1',
        cellMinWidth : 95,
        page : true,
        height : "full-125",
        limits : [10,15,20,25],
...
/*    table.on('tool(saleChances)', function(obj){
    ...
    */
    ...
    table.on('tool(saleChances)',function (obj){
        if (obj.event == "dev"){
            openDevOrInfoDialog("计划数据开发",obj.data.id);
        }else if (obj.event =="info"){
            openDevOrInfoDialog("计划数据维护",obj.data.id);
        }
    });
 ...
 /*   //打开客户开发计划的窗口
    function openDevOrInfoDialog(title,sId){
        layui.layer.open({
            ...
            area:["750px","550px"],
            maxmin: true,
            content:ctx + "/cus_dev_plan/plan_data?sId="+sId
        });
    }*/
